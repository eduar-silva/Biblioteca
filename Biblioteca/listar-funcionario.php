	...
	if($qtd > 0){
		...
		while($row = $res->fetch_object()){
			...
			print "<td>
					...
					<button class='btn btn-danger' onclick=\"if(confirm('Tem certeza que deseja excluir?')){location.href='?page=salvar-funcionario&acao=excluir&id_funcionario={$row->id_funcionario}';}else{false;}\">
						Excluir</button>
					</td>";
			 print "</tr>"; 
		}
